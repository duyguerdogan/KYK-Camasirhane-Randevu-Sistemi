...
namespace KykCamasirhaneSistemi
{
    public partial class CamasirMakinesi : Form
    {
        ...
        {
            string bosQuery = "SELECT CamasirMakinesiNo FROM CamasirMakinesiDurumu WHERE Dolu = 0";
            string doluQuery = "SELECT CamasirMakinesiNo FROM CamasirMakinesiDurumu WHERE Dolu = 1";

            List<int> bosMakineler = GetMakineList(bosQuery);
            List<int> doluMakineler = GetMakineList(doluQuery);

            label2.Text = "Boş Çamaşır Makineleri: " + string.Join(", ", bosMakineler);
            label3.Text = "Dolu Çamaşır Makineleri: " + string.Join(", ", doluMakineler);
        }
        ...
        {
            string query = "SELECT CamasirMakinesiNo FROM CamasirMakinesiDurumu WHERE Dolu = 0";
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (secimID > 0)
            {
                ...
                if (rezervasyonID > 0)
                {
                    AddRezervasyonTarihSaat(secimID, tarih, baslangicSaat, bitisSaat);
        ...
        {
            string query = "INSERT INTO MakineSecimi (CamasirMakinesiNo) OUTPUT INSERTED.SecimID VALUES (@MakineNo)";
            int secimID = 0;
        ...
        private void AddKullaniciSecimi(int kullaniciID, int secimID)
        {
            string query = "INSERT INTO KullaniciSecimi (KullaniciID, SecimID) VALUES (@KullaniciID, @SecimID)";
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                {
                    MessageBox.Show("Kullanıcı seçimi eklenirken hata oluştu: " + ex.Message);
                }
            }
        }

        private void AddRezervasyonTarihSaat(int secimID, DateTime tarih, TimeSpan baslangicSaat, TimeSpan bitisSaat)
        {
            
            string rezervasyonQuery = "SELECT RezervasyonID FROM Rezervasyon WHERE SecimID = @SecimID";

            int rezervasyonID = 0;

            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                SqlCommand command = new SqlCommand(rezervasyonQuery, connection);
                command.Parameters.AddWithValue("@SecimID", secimID);
                ...
                try
                {
                    ...
                    if (result != null)
                    {
                        rezervasyonID = Convert.ToInt32(result);
                    }
                    else
                    {
                        MessageBox.Show("RezervasyonID bulunamadı.");
                        return;
                    }
                }
                catch (Exception ex)
                {
                    MessageBox.Show("RezervasyonID alınırken hata oluştu: " + ex.Message);
                    return;
                }
            }
            ...
            using (SqlConnection connection = new SqlConnection(connectionString))
            {
                ...
                command.Parameters.AddWithValue("@BaslangicSaati", baslangicSaat); 
                command.Parameters.AddWithValue("@BitisSaati", bitisSaat); 
        ...
        {
            string query = "UPDATE CamasirMakinesiDurumu SET Dolu = 1 WHERE CamasirMakinesiNo = @MakineNo";
